...
let months = {}; // { "2025-10": { salary, fixedExpenses:[], unexpectedExpenses:[], alerts:[] } }
let alertsConfig = {
  percentage: 80 // alerta por defecto al alcanzar el 80% del salario
};

function monthKey(month, year) { return `${year}-${(month+1).toString().padStart(2,"0")}`; }

// Inicialización meses en select y pestañas
...
function getMonthObj() {
  ...
  if (!months[key]) {
    months[key]={salary:0,fixedExpenses:[],unexpectedExpenses:[],alerts:[]};
  }
  return months[key];
}
...
function renderFixed() {
  ...
  obj.fixedExpenses.forEach((exp,i) => {
    ...
    li.innerHTML = `<span>${exp.name}: ${fmtCOP.format(exp.amount)}${fechaLim}</span>
...
function renderUnexpected() {
  ...
  obj.unexpectedExpenses.forEach((exp,i) => {
    const li = document.createElement('li');
    li.innerHTML = `<span>${exp.name}: ${fmtCOP.format(exp.amount)}</span>
      <span class="gasto-actions">
...
function renderAlerts() {
  ...
  ul.innerHTML = '';
  // Alerta de fecha límite y porcentaje sobre salario
  obj.fixedExpenses.forEach(exp=>{
    let vencida = exp.date && (new Date(exp.date) < new Date());
    if (vencida) {
      const li = document.createElement('li');
  ...
  // Alerta general por porcentaje total de gasto sobre salario
...
let currentType = 'fixed', editIdx = null;
...
form.onsubmit = function(e) {
  ...
  let obj=getMonthObj();
  const nuevo = {name, amount, date, alertPct};
